...
const SeasonalEvent: React.FC<SeasonalEventProps> = ({ language, onBack }) => {
    ...
    return (
        <div className="w-full min-h-screen bg-[#0f172a] relative overflow-hidden touch-none">
            ...
            <div
                className="absolute inset-0 z-10 pointer-events-none flex flex-col p-4 gap-3 md:gap-4"
                style={{
                    paddingTop: 'max(1rem, env(safe-area-inset-top))',
                    paddingBottom: 'max(1rem, env(safe-area-inset-bottom))'
                }}
            >
                <div className="pointer-events-auto max-w-7xl mx-auto w-full flex flex-col gap-3 md:gap-4">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-4">
                        {!infoDismissed && (
                            <div className="relative w-full md:w-fit">
                                <div className="bg-gradient-to-r from-indigo-900/85 to-purple-900/85 backdrop-blur-xl border border-white/10 rounded-2xl p-3 md:p-4 shadow-[0_0_30px_rgba(99,102,241,0.3)] flex flex-col gap-1 animate-in slide-in-from-top-4 duration-700">
                                    <div className="flex items-center gap-2 text-cyan-300 border-b border-white/10 pb-1.5 mb-1">
                                        ...
                                        <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-[0.18em]">Сезон 2026</span>
                                    </div>
                                    ...
                                    <button
                                        className="absolute right-2 top-2 md:hidden w-7 h-7 flex items-center justify-center rounded-full border border-white/10 text-white/80 hover:text-white hover:bg-white/10 transition"
                ...
                <div className="pointer-events-auto w-full">
                    <div
                        className="w-full max-w-3xl bg-black/70 backdrop-blur-xl border border-white/10 md:rounded-3xl md:mx-auto md:static fixed left-0 right-0 bottom-0 rounded-t-3xl p-4 shadow-2xl"
                        style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))' }}
                    >
                        <div className="flex justify-center gap-2 mb-3 md:mb-4 overflow-x-auto touch-pan-x px-1">
                            {['decor', 'lights', 'top'].map((tab) => (
                                <button key={tab} onClick={() => setActiveTab(tab as any)} className={`px-3 py-2 md:px-4 md:py-2 rounded-full text-[10px] md:text-xs font-bold uppercase transition-all flex-shrink-0 ${activeTab === tab ? 'bg-pink-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
                                    {TABS[tab as keyof typeof TABS][language]}
                                </button>
                            ))}
                        </div>
                        <div className="flex gap-3 overflow-x-auto pb-1 md:pb-2 custom-scrollbar touch-pan-x">
