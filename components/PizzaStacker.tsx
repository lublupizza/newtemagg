...
const StackBox = React.memo(({ position, size, color, isTop }: { position: [number, number, number], size: [number, number], color: string, isTop?: boolean }) => {
    // Memoize material to prevent creation on every render
    ...
    return (
        <group position={position}>
            <RoundedBox args={[size[0], BOX_HEIGHT, size[1]]} radius={0.05} smoothness={4}>
            ...
            {isTop && (
                <mesh position={[0, BOX_HEIGHT/2 + 0.01, 0]} rotation={[-Math.PI/2, 0, 0]} geometry={planeGeo} material={logoMat} scale={[size[0]*0.6, size[1]*0.6, 1]} />
            )}
        </group>
    );
});

const FallingDebris = React.memo(({ data }: { data: DebrisState }) => {
    ...
    return (
        <group ref={ref} position={data.pos}>
            <mesh geometry={boxGeo} material={material} scale={[data.size[0], BOX_HEIGHT, data.size[1]]} />
        </group>
    );
});

const MovingPiece = React.memo(({ position, size, color }: { position: [number, number, number], size: [number, number], color: string }) => {
...
const BasePlatform = React.memo(() => {
    ...
    return (
        <group position={[0, -2, 0]}>
            ...
            {[1, -1].map(x => [1, -1].map(z => (
                <group key={`${x}-${z}`} position={[x*2.5, 0, z*2.5]}>
                    <mesh geometry={cylinderGeo} material={propMat} scale={[0.2, 0.5, 0.2]} />
                    <mesh position={[0, 0.3, 0]} rotation={[0, Date.now(), 0]} geometry={boxGeo} material={bladeMat} scale={[3, 0.05, 0.2]} />
                </group>
            )))}
            <Float speed={2} rotationIntensity={0.1} floatIntensity={0.5}>
                <group position={[0, 2, 0]}>
                    <mesh receiveShadow geometry={boxGeo} material={padMat} scale={[3.5, 0.5, 3.5]} />
                    <mesh position={[0, 0.26, 0]} rotation={[-Math.PI/2, 0, 0]} geometry={planeGeo} material={neonMat} scale={[3, 3, 1]} />
                </group>
            </Float>
        </group>
    )
...
const GameScene = ({ isPlaying, stack, debris, activeBox, cameraTargetY }: any) => {
    ...
    const activePos = activeBox.axis === 'x' 
        ? [activeBox.pos, activeBox.level * BOX_HEIGHT, lastBox.pos[2]]
        : [lastBox.pos[0], activeBox.level * BOX_HEIGHT, activeBox.pos];
    ...
    return (
        <>
            ...
            <BasePlatform />
            {stack.map((box: any, i: number) => (
                <StackBox key={box.id} position={box.pos} size={box.size} color={box.color} isTop={i === stack.length - 1} />
            ))}
            {debris.map((d: any) => <FallingDebris key={d.id} data={d} />)}
            {isPlaying && <MovingPiece position={activePos as any} size={activeBox.size} color={COLORS[activeBox.level % COLORS.length]} />}
        </>
    );
};
...
const PizzaStacker: React.FC<PizzaStackerProps> = ({ onGameOver, language, autoStart }) => {
    ...
    useEffect(() => {
        if (autoStart && gameState === 'start') startGame();
    }, [autoStart]);
    ...
    const handlePlace = () => {
        ...
        let debrisData: DebrisState | null = null;

        if (absDelta < PERFECT_TOLERANCE) {
            setCombo(c => c + 1);
            setScore(s => s + 10 + (combo + 1) * 5);
            ...
            debrisData = {
                id: Date.now(), pos: debrisPos, size: debrisSize, color: '#ef4444',
                vel: [isX ? sign * 0.1 : 0, 0, isX ? 0 : sign * 0.1],
                rotVel: [Math.random()*0.1, Math.random()*0.1, Math.random()*0.1]
            };
        }
    ...
    return (
        <div className="w-full h-[600px] relative bg-gray-950 rounded-3xl overflow-hidden border-4 border-pink-500 shadow-2xl select-none">
            ...
            <div className="absolute top-6 left-6 flex items-center gap-4 pointer-events-none">
                <h3 className="text-3xl font-black italic text-white drop-shadow-[0_0_10px_rgba(236,72,153,0.8)] neon-text">
                    Люблю<span className="text-pink-500">Pizza</span>
                </h3>
                <div className="bg-pink-600/20 border border-pink-500/50 px-3 py-1 rounded-lg text-pink-400 font-mono font-bold text-xl flex items-center gap-2">
