...
import { Trophy, Crown, TrendingUp, Search, Gamepad2, Gem, Globe, Timer, LayoutGrid } from 'lucide-react';
...
// --- SVG BADGES (Reusable) ---
const RankBadge = React.memo(({ rank, className }: { rank: number, className?: string }) => {
...
const PodiumPlayer = React.memo(({ user, place, t, themeColor }: { user: RankUser, place: number, t: any, themeColor: string }) => {
    ...
    let accentColor = 'from-yellow-600/40 to-yellow-900/60'; 
    ...
    return (
        <div className={`relative flex flex-col items-center justify-end group z-10 min-w-[120px] ${place === 1 ? 'order-2 -mt-12 z-20' : place === 2 ? 'order-1' : 'order-3'}`}>
            ...
            <div className={`w-full min-w-[110px] md:min-w-[160px] ${heightClass} rounded-t-[30px] bg-gradient-to-b ${accentColor} border-t border-x ${borderColor} backdrop-blur-xl flex flex-col items-center pt-12 pb-4 px-2 text-center transition-all duration-500 ${glow} hover:bg-opacity-80 animate-in slide-in-from-bottom-20 fade-in duration-1000 fill-mode-backwards ${delay}`}>
                <h3 className={`text-lg md:text-xl font-black text-white truncate w-full px-2 ${isFirst ? 'text-yellow-200' : ''}`}>{user.name}</h3>
                <div className="mt-1 flex items-center gap-1 text-[10px] uppercase tracking-widest text-white/60 font-mono">{t.categories[user.category]}</div>
                <div className="mt-auto mb-4 w-full">
                     <div className="text-[10px] text-white/40 uppercase tracking-widest mb-1">{t.points}</div>
                     <div className={`text-2xl md:text-3xl font-black font-mono ${isFirst ? 'text-yellow-400' : 'text-white'}`}>{user.score.toLocaleString()}</div>
                </div>
    ...
    <div className="min-h-screen bg-[#0B0F19] text-white overflow-x-hidden pb-20 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10 pt-8">
        <div className="flex flex-col items-center mb-12 text-center">
            ...
            <h1 className="text-5xl md:text-7xl font-black italic text-transparent bg-clip-text bg-gradient-to-b from-white via-gray-200 to-gray-500 mb-2 drop-shadow-[0_0_30px_rgba(255,255,255,0.2)] animate-in zoom-in duration-700">{t.title}</h1>
            ...
            <div className="w-full max-w-2xl flex flex-col md:flex-row items-center gap-4 p-2 bg-gray-900/60 backdrop-blur-md border border-white/10 rounded-2xl shadow-xl animate-in fade-in slide-in-from-bottom-4 delay-300 duration-700">
                <div className="flex gap-1 bg-black/20 p-1 rounded-xl w-full md:w-auto">
                   {[
                     ...
                     <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex-1 md:flex-none px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all ${activeTab === tab.id ? tab.id === 'gamer' ? 'bg-pink-600 text-white shadow-lg shadow-pink-500/40' : tab.id === 'collector' ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/40' : 'bg-gray-700 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}><tab.icon className="w-3 h-3" /> {tab.label}</button>
                   ))}
                </div>
                <div className="relative w-full">
                   <Search className="absolute left-3 top-1/2 -translate-x-0 -translate-y-1/2 text-gray-500 w-4 h-4" />
                   <input type="text" placeholder={t.search} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-black/20 border border-white/5 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white focus:outline-none focus:border-white/20 transition-colors placeholder-gray-600" />
                </div>
            </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
           <div className={`bg-gray-900/40 border rounded-3xl overflow-hidden backdrop-blur-md shadow-2xl transition-colors duration-500 ${activeTab === 'gamer' ? 'border-pink-500/20' : activeTab === 'collector' ? 'border-amber-500/20' : 'border-white/5'}`}>
               ...
               <div className="divide-y divide-white/5">
                   {rest.map((user, idx) => (
                       <div key={user.id} className="grid grid-cols-12 px-6 py-4 items-center hover:bg-white/5 transition-all duration-300 group animate-in fade-in slide-in-from-bottom-4 fill-mode-backwards" style={{ animationDelay: `${500 + idx * 50}ms` }}>
                           <div className="col-span-2 flex justify-center"><div className="w-8 h-8 rounded-lg bg-gray-800/50 flex items-center justify-center font-mono font-bold text-gray-400 group-hover:text-white group-hover:bg-gray-700 transition-colors">{user.rank}</div></div>
                           <div className="col-span-5 flex items-center gap-3">
                               <div className={`w-10 h-10 rounded-full ${user.avatarColor} flex items-center justify-center border-2 border-white/10 shadow-md`}><span className="font-bold text-white text-xs">{user.name[0]}</span></div>
                               <div className="flex flex-col"><span className="font-bold text-gray-200 group-hover:text-white transition-colors">{user.name}</span><span className="text-[10px] text-gray-500 uppercase tracking-wider flex items-center gap-1"><span className={`w-2 h-2 rounded-full ${user.category === 'gamer' ? 'bg-pink-500' : 'bg-amber-500'}`}></span>{t.categories[user.category]}</span></div>
                           </div>
                           <div className="col-span-3 hidden md:flex justify-center items-center gap-4 text-gray-500">
                               <div className="flex items-center gap-1" title="Achievements"><Trophy className="w-3 h-3" /> <span className="text-xs font-mono">{user.achievements}</span></div>
                               <div className="flex items-center gap-1" title="Trend">{user.trend === 'up' ? <TrendingUp className="w-3 h-3 text-green-500" /> : user.trend === 'down' ? <TrendingUp className="w-3 h-3 text-red-500 rotate-180" /> : <div className="w-3 h-1 bg-gray-600 rounded-full"></div>}</div>
                           </div>
                           <div className="col-span-5 md:col-span-2 text-right"><span className={`font-mono font-black text-lg transition-colors block ${activeTab === 'gamer' ? 'text-gray-300 group-hover:text-pink-400' : activeTab === 'collector' ? 'text-gray-300 group-hover:text-amber-400' : 'text-gray-300 group-hover:text-white'}`}>{user.score.toLocaleString()}</span></div>
                       </div>
                   ))}
               </div>
           </div>
